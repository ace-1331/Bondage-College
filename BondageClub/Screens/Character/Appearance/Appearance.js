...
function AppearanceClick() {
	...
	if (!CharacterAppearanceWardrobeMode && CharacterAppearanceColorPicker == "") {
		...
		if (CommonIsClickAt(1210, 145, 65, 830))
			for (var A = CharacterAppearanceOffset; A < AssetGroup.length && A < CharacterAppearanceOffset + CharacterAppearanceNumPerPage; A++)
		...
		if (CommonIsClickAt(1300, 145, 400, 830))
			for (var A = CharacterAppearanceOffset; A < AssetGroup.length && A < CharacterAppearanceOffset + CharacterAppearanceNumPerPage; A++)
		...
		if (CommonIsClickAt(1725, 145, 160, 830))
			for (var A = CharacterAppearanceOffset; A < AssetGroup.length && A < CharacterAppearanceOffset + CharacterAppearanceNumPerPage; A++)
				if ((AssetGroup[A].Family == C.AssetFamily) && (AssetGroup[A].Category == "Appearance") && (C.ID == 0 || AssetGroup[A].Clothing))
					if ((MouseY >= 145 + (A - CharacterAppearanceOffset) * 95) && (MouseY <= 210 + (A - CharacterAppearanceOffset) * 95))
		...
		if (CommonIsClickAt(1910,145, 65, 830))
			for (var A = CharacterAppearanceOffset; A < AssetGroup.length && A < CharacterAppearanceOffset + CharacterAppearanceNumPerPage; A++)
		...
		if (CommonIsClickAt(1300, 25, 90, 90) && (C.ID == 0) && !LogQuery("Wardrobe", "PrivateRoom")) CharacterAppearanceSetDefault(C);
		if (CommonIsClickAt(1300, 25, 90, 90) && (C.ID == 0) && LogQuery("Wardrobe", "PrivateRoom")) CharacterAppearanceWardrobeLoad(C);
		if (CommonIsClickAt(1417, 25, 90, 90) && (C.ID == 0)) CharacterAppearanceFullRandom(C);
		if (CommonIsClickAt(1417, 25, 90, 90) && (C.ID != 0) && LogQuery("Wardrobe", "PrivateRoom")) CharacterAppearanceWardrobeLoad(C);
		if (CommonIsClickAt(1534, 25, 90, 90)) CharacterAppearanceStripLayer(C);
		if (CommonIsClickAt(1651, 25, 90, 90)) CharacterAppearanceMoveOffset(C, CharacterAppearanceNumPerPage);
		if (CommonIsClickAt(1768, 25, 90, 90)) CharacterAppearanceExit(C);
		...
		if (CommonIsClickAt(1651, 25, 90, 90)) {
		...
		if (CommonIsClickAt(1417, 25, 90, 90)) { CharacterAppearanceWardrobeMode = false; ElementRemove("InputWardrobeName"); }
		if (CommonIsClickAt(1534, 25, 90, 90)) CharacterAppearanceStripLayer(C);
		if (CommonIsClickAt(1768, 25, 90, 90)) CharacterAppearanceExit(C);
		if (CommonIsClickAt(1885, 25, 90, 90)) CharacterAppearanceReady(C);
		...
		if (CommonIsClickAt(1768, 25, 90, 90)) {
